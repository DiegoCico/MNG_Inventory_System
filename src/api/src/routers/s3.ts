...
function requireBucket(): string {
  const bucket =
    process.env.S3_BUCKET || (process.env.NODE_ENV === 'test' ? 'unit-test-bucket' : '');
...
function getTeamId(ctx: S3Ctx): string {
  ...
  const fallbackForTests =
    process.env.NODE_ENV === 'test' ? (process.env.TEST_TEAM_ID ?? 'alpha') : undefined;
...
function prefixFor(
  teamId: string,
  args: Pick<z.infer<typeof UploadInput>, 'scope' | 'itemId' | 'serialNumber'>,
): string {
...
function buildKey(teamId: string, args: z.infer<typeof UploadInput>, mime: string): string {
  const id = randomUUID();
...
export const s3Router = router({
  ...
  uploadImage: publicProcedure.input(UploadInput).mutation(async (opts) => {
  ...
  getSignedUrl: publicProcedure.input(GetUrlInput).query(async (opts) => {
  ...
  deleteObject: publicProcedure.input(DeleteInput).mutation(async (opts) => {
